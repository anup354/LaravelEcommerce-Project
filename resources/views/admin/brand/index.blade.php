...
@section('category_select', 'bg-[#4456a6] text-white')
@section('body')
                    ...
                    @foreach ($brands as $key => $brand)
                        <tbody class="bg-white divide-y divide-gray-200 text-center">
                            <tr>
                                ...
                                <td class="">
                                    <div>{{ $brand->brand_order }}</div>
                                </td>

                                <td class="">
                                    <div>{{ $brand->featured ? 'Yes' : 'No' }}</div>
                                </td>
                                {{-- <td class="">
                                ...
                                <td>
                                    @if ($brand->created_at)
                                        <div>{{ $brand->created_at->format('F j, Y') }}</div>
                                    @else
                                        <div>-</div>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="flex p-2 justify-center">
                                        ...
                                        <form method="POST" action="{{ route('brand.destroy', $brand->id) }}"
                                            ...
                                            <button type="button" onclick="deleteSingleImage({{ $brand->id }})"
